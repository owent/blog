...
最近还看到另一个库[call_in_stack][5]，作者说是创建栈的时候不要对齐到4KB，这样能减少分支预测的miss，能大幅提高协程效率（他说是大约10ns左右，我表示怀疑，刚刚测试的时候发现[call_in_stack][5]在我这里[CentOS 7+GCC4.8]并不能编译过，所以）。我打算过段时间也去研究下这个。术语好像叫*stride access*，等有空研究下。但是他这个库并没有考虑线程安全、资源管理之类的东西，还需要额外的实现。而我们游戏中使用协程其实很容易cache miss的。因为逻辑必然比协程切换要复杂，几乎必然cache miss。所以我这里的压力测试结果和[libgo][2]差不太多，比它稍微好一些。

> 测试结果简报: *-O2情况下，低cache miss协程切换约需要60ns，高cache miss需要约260ns，同时协程任务和action的维护代价约是40ns*。也就是说如果使用完整的协程任务+栈池。那么实际场景中的切换开销约300ns。简要的profile如下:
...
C++1z后面可能C++会内置支持co_await之类的关键字了。我最近也在抽空看它的原理和文档。后面有时间我也会做一下这些的集成和支持的。不过这个还没那么快，只是在TODO LIST里。

还有就i是前面提到的分支预测的优化，我也需要再找点资料。再评估一下，看看有没有比较搞进去。
