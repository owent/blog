# 2016

  * [重写了llvm+clang+libc++和libc++abi的构建脚本](重写了llvm+clang+libcxx+libcxxabi的构建脚本.md)
  * [理解Raft算法](理解Raft算法.md)
  * [游戏服务器的不停服更新](游戏服务器的不停服更新.md)
  * [接入letsencrypt+全面启用HTTP/2](接入letsencrypt和全面启用HTTP2.md)
  * [对atbus的小数据包的优化](对atbus的小数据包的优化.md)
  * [博客文章和文档迁移到gitbook](博客文章和文档迁移到gitbook.md)
  * [pbc的一个陈年老BUG](pbc的一个陈年老BUG.md)
  * [libatbus基本功能及单元测试终于写完啦](libatbus基本功能及单元测试终于写完啦.md)
  * [boost.context-1.61版本的设计模型变化](boost.context-1.61版本的设计模型变化.md)
  * [atsf4g完整游戏工程示例](atsf4g完整游戏工程示例.md)
  * [atframework基本框架已经完成](atframework基本框架已经完成.md)
  * [Android和IOS的TLS问题](Android和IOS的TLS问题.md)
